	...
	*/
	public function del(){
		...
		foreach($attr_value as $k => $v){
			$tmp = M('attr_spec_price_store')->where('attr_value_id="'.$v['id'].'"')->select();
			if(!empty($tmp)){
		...
		if ($res) {
			M('attr_value')->where('attr_id="'.intval($id).'"')->delete();
			$this->success('操作成功.');
		}else{
			$this->error('操作失败.');
		}
	...
	public function create(){
		...
		if($data['attr_id'] == ''){
			$tmp['attr_name'] = $data['attr_name'];
			// $tmp['cate_id'] = $data['cate'];
			$bool = M('attribute')->where('attr_name="'.$data['attr_name'].'"')->select();
			if(!empty($bool)){
				$this->error('该分类下已经存在该属性！');
				exit;
			}
			$tmp['sort'] = $data['sort'];
			$tmp['attr_value'] = $data['attr_value'];
			$tmp['addtime'] = time();
			$num = $attr->add($tmp);
			if($num){
				$data['attr_value'] = explode(',',$data['attr_value']);
				foreach($data['attr_value'] as $k => $v){
					$tmp2['attr_id'] = M('attribute')->where('attr_name="'.$data['attr_name'].'"')->getField('id');
					$tmp2['name'] = $v;
					$attr_value->add($tmp2);
					unset($tmp2);
				}
			...
			$tmp['attr_value'] = $data['attr_value'];
			$tmp['addtime'] = time();
			$num = $attr->where('id="'.$data['attr_id'].'"')->save($tmp);
			if($num){
				$attr_value->where('attr_id="'.$data['attr_id'].'"')->delete();
				$data['attr_value'] = explode(',',$data['attr_value']);
				foreach($data['attr_value'] as $k => $v){
					$tmp3['attr_id'] = $data['attr_id'];
					$tmp3['name'] = $v;
					$attr_value->add($tmp3);
					unset($tmp3);
				}
