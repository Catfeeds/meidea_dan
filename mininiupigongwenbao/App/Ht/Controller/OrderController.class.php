...
class OrderController extends PublicController{
	...
	*/
	...
	public function comment(){
		...
		$where = "uid > 0";
		if($order_sn != null){
			$order_id = $order->where('order_sn="'.$order_sn.'"')->getField('id');
			if(!empty($order_id)){
		...
		if($pname != null){
			$pid = $product->where('name="'.$pname.'"')->getField('id');
			if(!empty($pid)){
				$where .= ' AND pid ='.$pid;
			}else{
				$where .= ' AND pid = 0';
		...
		$res = $comment->where($where)->select();
		$user = M('user');
		foreach($res as $k => $v){
			$res[$k]['uname'] = $user->where('id='.$v['uid'])->getField('uname');
			$res[$k]['pname'] = $product->where('id='.$v['pid'])->getField('name');
			$res[$k]['order'] = $order->where('id='.$v['order_id'])->getField('order_sn');
		}
	...
	public function replyComment(){
		...
		$comment = M('product_dp');
		$user = M('user');
		$res = $comment->where('id="'.$cid.'"')->select();
		$res[0]['uname'] = $user->where('id="'.$res[0]['uid'].'"')->getField('uname');
		$this->assign('comment', $res);
		$this->display();
	}
	...
	public function reply(){
		...
		$num = $comment->where('id="'.$cid.'"')->data($data)->save();
		if($num){
			$this->redirect('Order/replyComment', array('cid' => $cid));
		}else{
			$this->error('回复失败！');
		}
	}
	...
	public function delComment(){
		$cid = $_GET['id'];
		$comment = M('product_dp');
		$num = $comment->where('id="'.$cid.'"')->delete();
		if($num){
			$this->success('删除成功！');
		}else{
			$this->error('删除失败！');
