...
class OrderController extends PublicController {
	...
	public function order_details(){
		...
		$order_info = $orders->where('id='.intval($order_id.' AND del=0'))->field('id,order_sn,shop_id,status,addtime,price,type,post,tel,receiver,address_xq,remark,kuaidi_name,kuaidi_num')->find();
		if (!$order_info) {
	...
	public function change_order(){
		...
		$re=M("order")->where("id=$orderid AND uid=$uid AND shop_id=$shopid")->save($data);
		if($re){
	...
	public function sendComment(){
		...
		$num = $comment->add($data);
		if($num){
			echo json_encode(array("status"=>1,"error"=>"评价成功!"));
		}else{
			echo json_encode(array("status"=>0,"error"=>"评价失败!"));
		}
	}
